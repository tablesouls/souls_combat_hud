...
public class IronsSpellsProvider {
    public static SpellSelectionManager getManager() {
        if (!IronsSpellsCompat.LOADED) return null;
    ...
    public static void cycle() {
        if (!IronsSpellsCompat.LOADED) return;
        SpellSelectionManager manager = getManager();
        if (manager == null || manager.getSpellCount() <= 1) {
            return;
        }
        SoundHelper.playUiSound(ModSounds.CYCLE_SPELL);
